...
class PCGA:
    ...
    def __init__(self, forward_model, s_init, pts, params, s_true=None, obs=None, obs_true=None, X=None):
        ...
        if self.post_cov or self.post_cov == "diag":
            ...
            if 'post_diag_direct' in params:
                ...
                if self.post_diag_direct:
                    print(
                        "WARNING!! : you chose to perform direct posterior variance analysis, which would take forever! You can use post_diag_direct=False")
        ...
        self.forward_model_verbose = True if 'forward_model_verbose' not in self.params else self.params[
            'forward_model_verbose']
    ...
    def ComputePriorEig(self, n_pc=None):
        ...
        print('- 1st eigv : %g, %d-th eigv : %g, ratio: %g' % (
            self.priord[0], self.n_pc, self.priord[-1], self.priord[-1] / self.priord[0]))
        return
    ...
    def JacVect(self, x, s, simul_obs, precision, delta=None):
        ...
        if delta is None or isnan(delta) or delta == 0:
            for i in range(nruns):
                ...
                deltas[i] = signmag * sqrt(precision) * (max(abs(mag), absmag)) / (
                        (np.linalg.norm(x[:, i:i + 1]) + np.finfo(float).eps) ** 2)

                if deltas[i] == 0:  # s = 0 or x = 0
                    print("%d-th delta: signmag %g, precision %g, max abs %g, norm %g" % (
                        i, signmag, precision, (max(abs(mag), absmag)), (np.linalg.norm(x) ** 2)))
        ...
        if np.size(simul_obs_purturbation, 1) != nruns:
            raise ValueError("size of simul_obs_purturbation (%d,%d) is not nruns %d" % (
                simul_obs_purturbation.shape[0], simul_obs_purturbation.shape[1], nruns))
    ...
    def ObjectiveFunctionNoBeta(self, s_cur, simul_obs, approx=True):
        ...
        if approx:
            ...
            tmp = np.linalg.solve(XTinvQX,
                                  XTinvQs)  # inexpensive solve p by p where p <= 3, usually p = 1 (scalar devision)
            obj = 0.5 * np.dot(ymhs.T, np.divide(ymhs, self.R)) + 0.5 * (
            ...
            obj = 0.5 * np.dot(ymhs.T, np.divide(ymhs, self.R)) + 0.5 * (
                    np.dot(s_cur.T, invQs) - np.dot(XTinvQs.T, tmp))
        return obj
    ...
    def DirectSolve(self, s_cur, simul_obs=None):
        ...
        LM_eval = np.zeros(nopts,
                           dtype=bool)  # if LM_smax, LM_smin defined and solution violates them, LM_eval[i] becomes True
                ...
                for i in range(nopts):
                    if i == 0:
                        ...
                        if LM_eval[i]:
                            ...
                            simul_obs_all = np.concatenate((simul_obs_all, self.ForwardSolve(s_hat_all[:, i:i + 1])),
                                                           axis=1)
        else:
        ...
        for i in range(nopts):
            if self.objeval:  # If true, we do accurate computation
                if LM_eval[i]:
                    ...
                    obj = self.ObjectiveFunction(s_hat_all[:, i:i + 1], beta_all[:, i:i + 1], simul_obs_all[:, i:i + 1],
                                                 approx=False)
    ...
    def IterativeSolve(self, s_cur, simul_obs=None, precond=False):
        ...
        print('### IMPORTANT VALUES ###', m, n, p, n_pc, R)
        ...
        print ('JACMAT INPUT VALUES', s_cur, simul_obs, Z)
        print ('JACMAT OUTPUT VALUES', HX, HZ, Hs, U_data)
        # debug_here()
        ...
        if R.shape[0] == 1:  # Compute eig(P*(HQHT+R)*P) approximately by svd(P*HZ)**2 + R if R is single number
            ...
            if self.verbose:
                print("eig. val. of generalized data covariance : %f secs (%8.2e, %8.2e, %8.2e)" % (
        ...
        if self.precond:
            ...
            if R.shape[0] == 1:
                ...
                # n by n
                def pmv(v):
                    return np.multiply(self.invsqrtR.reshape(v.shape),
                                       np.dot(HZ, (np.dot(HZ.T, np.multiply(self.invsqrtR.reshape(v.shape), v)))))
        ...
        for i in range(nopts):  # this is sequential for now
            ...
            if R.shape[0] == 1:
                def mv(v):
                    return np.concatenate(((np.dot(HZ, np.dot(HZ.T, v[0:n])) + np.multiply(np.multiply(alpha[i], R),
                                                                                           v[0:n]) + np.dot(HX, v[
                                                                                                                n:n + p])),
                                           (np.dot(HX.T, v[0:n]))), axis=0)
            else:
                def mv(v):
                    return np.concatenate(((np.dot(HZ, np.dot(HZ.T, v[0:n])) + np.multiply(
                        np.multiply(alpha[i], R.reshape(v[0:n].shape)), v[0:n]) + np.dot(HX, v[n:n + p])),
                                           (np.dot(HX.T, v[0:n]))), axis=0)
            ...
            if self.precond:
                ...
                if R.shape[0] == 1:
                    ...
                    def invPsi(v):
                        ...
                        if Psi_UTv.ndim == 1:
                            PsiDPsiTv = np.dot(Psi_U_i,
                                               np.multiply(Dvec[:Psi_U_i.shape[1]].reshape(Psi_UTv.shape), Psi_UTv))
                            ...
                            raise ValueError(
                                "Psi_U times vector should have a dimension smaller than 2 - current dim = %d" % (
                                    Psi_UTv.ndim))
                    ...
                    def invPsi(v):
                        ...
                        if Psi_UTv.ndim == 1:
                            ...
                            RMat = np.tile(np.multiply((1. / alpha[i]), self.invR), Psi_UTv.shape[
                                1])  # may need to change this later in a more general way 06142018 Harry
                ...
                def Pmv(v):
                    ...
                    return np.concatenate(
                        ((invPsiv - invPsiHXinvSHXTinvPsiv + invPsiHXinvSv1), (invSHXTinvPsiv - invSv1)), axis=0)
                ...
                if info != 0:  # if not converged
                    ...
                    if self.verbose: print(
                        "-- Number of iterations for minres %g and info %d" % (callback.itercount(), info))
            else:
                ...
                if info != 0:
                    ...
                    print("-- Number of iterations for gmres: %g, info: %d, tol: %f" % (
            ...
            if self.verbose:
                if LM_eval[i]:
                    print("%d - min(s): %g, max(s) :%g - violate LM_smin or LM_smax" % (
                        i, s_hat_all[:, i:i + 1].min(), s_hat_all[:, i:i + 1].max()))
                else:
    ...
    def GaussNewton(self):
        ...
        print(
            'obs. RMSE (norm(obs. diff.)/sqrt(nobs)): %g, normalized obs. RMSE (norm(obs. diff./sqrtR)/sqrt(nobs)): %g' % (
                RMSE_init, nRMSE_init))
        ...
        for i in range(maxiter):
            ...
            print(s_cur, beta_cur, simul_obs_cur, obj)
            ...
            if self.s_true is not None:
                ...
                print("= objective function is %e, relative L2-norm diff btw sol %d and sol %d is %g" % (
                    obj, i, i + 1, res))
                print("= L2-norm error (w.r.t truth) is %g, obs. RMSE is %g, obs. normalized RMSE is %g" % (
                    err, RMSE_cur, nRMSE_cur))
            else:
                print("== iteration %d summary ==" % (i + 1))
                print("= objective function is %e, relative L2-norm diff btw sol %d and sol %d is %g" % (
                    obj, i, i + 1, res))
        ...
        print(
            "** Solution RMSE %g , initial RMSE %g, where RMSE = (norm(obs. diff.)/sqrt(nobs)), Solution nRMSE %g, init. nRMSE %g" % (
                np.linalg.norm(self.simul_obs_best - self.obs) / np.sqrt(self.n), RMSE_init,
                np.linalg.norm(np.divide(self.simul_obs_best - self.obs, self.sqrtR)) / np.sqrt(n)
                , nRMSE_init))
    ...
    def Run(self):
        start = time()
        if self.Q is None:
            self.ConstructCovariance(method=self.matvec, kernel=self.kernel, xmin=self.xmin, xmax=self.xmax, N=self.N,
                                     theta=self.prior_cov_scale)
        ...
        if R.shape[0] == 1:
            def invPsi(v):
                ...
                if Psi_UTv.ndim == 1:
                    ...
                    raise ValueError(
                        "Psi_U times vector should have a dimension smaller than 2 - current dim = %d" % (Psi_UTv.ndim))
            ...
            def invPsi(v):
                ...
                if Psi_UTv.ndim == 1:
                    ...
                    RMat = np.tile(np.multiply((1. / alpha[i_best]), self.invR), Psi_UTv.shape[
                        1])  # may need to change this later in a more general way 06142018 Harry
                    alphainvRv = np.multiply(RMat, v)
                    Dmat = np.tile(Dvec[:Psi_U.shape[1]], (Psi_UTv.shape[1], 1)).T  # n_pc by p
                    PsiDPsiTv = np.dot(Psi_U, np.multiply(Dmat, Psi_UTv))
